...
class AjustesMedidores(tk.Frame, object):
    ...
    def crear_elementos(self):
        self.rangoLabel = tk.Label(self.master, text="Rango", bg='white', font='Helvetica 10 bold')
        self.minimoEntry = tk.Entry(self.master, bd=5, width=5)
        self.minimoEntry.insert(0, self.minimo_rango)
        self.maximoEntry = tk.Entry(self.master, bd=5, width=5)
        self.maximoEntry.insert(0, self.maximo_rango)
        self.umbralRadioLabel = tk.Label(self.master, text="Tipo de umbral", bg='white', font='Helvetica 10 bold')
        self.umbralRadio0 = tk.Radiobutton(self.master, text="Porcentaje", variable=self.tipo_umbral, value='P',
                                           command=self.cambiar_umbral)
        self.umbralRadio1 = tk.Radiobutton(self.master, text="Valor", variable=self.tipo_umbral, value='V',
                                           command=self.cambiar_umbral)
        self.umbralLabel = tk.Label(self.master, text="Umbrales", bg='white', font='Helvetica 10 bold')
        self.umbral0 = tk.StringVar()
        self.umbralEntry0 = tk.Entry(self.master, textvariable=self.umbral0, bd=5, width=6)
        self.umbralEntry0.insert(0, self.umbrales_actual[0])
        self.umbral1 = tk.StringVar()
        self.umbralEntry1 = tk.Entry(self.master, textvariable=self.umbral1, bd=5, width=6)
        ...
        self.colorButton0.config(font='Helvetica 9 bold', text="< " + str(self.umbrales_actual[0]) + self.unidad_actual,
                                 bg=self.colores[0])
        self.colorButton1 = tk.Button(self.master, width="18", command=lambda: self.set_color(self.colorButton1, 1))
        self.colorButton1.config(font='Helvetica 9 bold',
                                 text=">= " + str(self.umbrales_actual[0]) + self.unidad_actual + " y < " + str(
                                     self.umbrales_actual[1]) + self.unidad_actual, bg=self.colores[1])
        self.colorButton2 = tk.Button(self.master, width="18", command=lambda: self.set_color(self.colorButton2, 2))
        self.colorButton2.config(font='Helvetica 9 bold',
                                 text=">= " + str(self.umbrales_actual[1]) + self.unidad_actual, bg=self.colores[2])
    ...
    def cambiar_umbral(self):
        """Cambia la interfaz según el tipo de umbral seleccionado"""
    ...
    def validar_entries(self, valor1, valor2):
        """Se previenen errores del usuario en las inputs"""
        if valor1 > valor2:
